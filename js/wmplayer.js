...
var WMPlayer = (function () {
	function WMPlayer(settings,callback) {
		var $this = this;
		init(callback);
		bindEvents();
		formatData();
		...
		$.each($this.list, function (i) {
			listHTML += '<li class="wm-list-title-'+ i +'">' + $this.list[i].listName + '</li>';
		});
		...
		function init (callback) {
			...
			$this.dom = {
				container:con,
				cover:con.find('.wm-cover'),
				name:con.find('.wm-name'),
				singer:con.find('.wm-singer'),
				currentTime:con.find('.wm-time-current'),
				allTime:con.find('.wm-time-all'),
				prev:con.find('.wm-prev'),
				play:con.find('.wm-pause'),
				next:con.find('.wm-next'),
				vol:con.find('.wm-vol-img'),
				volRange:con.find('.wm-vol-range'),
				progress:con.find('.wm-pro-current'),
				progressAll:con.find('.wm-pro'),
				lrc:con.find('.wm-lrc'),
				listTitle:con.find('.wm-list-title'),
				list:con.find('.wm-list')
			};
		...
		function bindEvents () {
			...
			$this.audio.on('canplay',function () {
				...
				// 更新歌词
				var dataLrc = parseInt($this._data('currentLrc'));
				...
				if (dataLrc != currentLrc) {
					...
					// 删除以前添加的class
					$this.dom.lrc.find('.wm-lrc-current').removeClass('wm-lrc-current');
					
					$this.dom.lrc.each(function(index, el) {
						el = $(el);
						// 添加class
						var position = el.find('.wm-lrc-time-'+currentLrc).addClass('wm-lrc-current').position();
						// 计算滚动的高度
						var positionTop = position ? position.top : 0;
						var top = el.scrollTop();
						var lrcTop = parseInt(el.attr('wm-top') || 0);
						var time = parseInt(el.attr('wm-time') || 0);
						// 动画滚动
						el.animate({
			...
			$this.dom.list.on('click',eleName, function () {
				$this.play(parseInt($this._data('displayList')),$(this).index());
			});
			// 进度条
			...
			// 禁止点击
			$this.dom.container.on('mousedown','.wm-disabled', function () {
				return false;
			})
		}
		 ...
		 */
		function next () {
			var mode = parseInt($this._data('playMode'));
			var songNum = $this.getSongNum();
			switch (mode) {
				case 0: // 顺序
					var currentSong = $this._data('currentSong');
		...
		function setTime (time,ele) {
			var minute = $this.util.fillByZero(Math.floor(time/60),2);
			var second = $this.util.fillByZero(Math.floor(time % 60),2);
			ele.html(minute + ':' + second);
		}
	}
	$.extend(WMPlayer.prototype,{
		/**
		 * 更换歌词
		 * @param lrc {String}
		 * @param [ajax] {Boolean}
		 * @private
		 */
		_setLrc: function (lrc,ajax) {
			var $this = this;
			$this.dom.lrc.html('').scrollTop(0);
			if (ajax === true) {
				$this._data({
					'ajaxList': $this.getCurrentList(),
					'ajaxSong': $this.getCurrentSong()
				});
				$.get(lrc, function (data,status) {
					if (status == 'success') {
						data = $this._parseLrc(data);
						var ajaxList = parseInt($this._data('ajaxList'));
						var ajaxSong = parseInt($this._data('ajaxSong'));
						$this.list[ajaxList][ajaxSong].ajax = false;
						$this.list[ajaxList][ajaxSong].lrc = data;
						// 判断是否在ajax期间切换过歌曲
						var currentList = $this.getCurrentList();
						var currentSong = $this.getCurrentSong();
						if (ajaxSong == currentSong && ajaxList == currentList) {
							$this._setLrc(data);
						}
					}
				},'text');
				...
				$.each(lrc, function (index,val) {
					$this.dom.lrc.append($('<li></li>').addClass('wm-lrc-time-'+index).html(val));
				});
			}
		},
		/**
		...
		_setCurrent: function (song) {
			...
			items.find('.wm-list-current').removeClass('wm-list-current');
			items.children().eq(song).addClass('wm-list-current');
		},
		/**
		 * 解析歌词字符串
		 * @param lrc
		 ...
		 * @private
		 */
		_data: function (attr,value) {
			if (value === undefined) {
				return parseInt(this.data.data(attr));
			} else {
				this.data.data(attr,value)
			}
		},
		/**
		 * 下一首
		...
		next: function () {
			...
			switch (mode) {
				case 2: // 随机
					var song = $this.util.rand(0,songNum-1);
					$this.play(song);
					break;
				case 0: // 顺序
				case 3: // 列表
		...
		pause: function () {
			var $this = this;
			$this.dom.play.removeClass('wm-play');
		 ...
		 * @param data.lrc {String}
		 * @param data.src {String}
		 * @param data.name {String}
		 * @param [data.singer] {String}
		 * @param [data.img] {String}
		 * @param [data.ajax] {Boolean}
		...
		/**
		 * 获取当前播放歌曲的信息
		 * @param [info=false] {boolean|} 可选，默认false，将返回歌曲id，填true可返回详细歌曲信息
		 * @returns {object|int}
		 */
		...
		/**
		 * 获取当前播放列表的信息
		 * @param [info=false] {boolean} 可选，默认false，将返回列表id，填true可返回详细列表信息
		 * @returns {object|int}
		 */
		getCurrentList: function (info) {
			...
			if (info) {
				...
				return parseInt($this._data('currentList'));
			}
		},
		/**
		 * 获取当前播放列表的信息
		...
		getPlayMode: function () {
			return parseInt(this._data('playMode'));
		},
		/**
		 * 获取是否静音
		 * @returns {boolean}
		...
		cancelMute: function () {
			...
			$this.dom.vol.removeClass('wm-mute');
			$this._trigger('mute');
		},
		/**
		 * 切换静音状态
